...
- `interface-synthesizer/0.2.json` — interface synthesizer contract. Defines `listBindingSpecs` and `synthesizeInterface` for components that produce OBIs from existing source artifacts.
- `source-inspector/0.1.json` — source inspector contract. Defines `listBindingSpecs` and `inspectSource` for components that inspect source artifacts and return bindable targets before an OBI is created. (New for spec 0.2.0; first contract version 0.1.0.)
- `document-store/0.1.json` — generic store of named JSON documents (`get`/`set`/`delete` over an opaque key and a whole JSON object, in the document-database sense). Generic capability; the runtime uses one to hold binding context, but the store knows nothing about context. (Replaces the spec-0.1.0 `context-store`, which baked the context meaning into the store.)
...
- `openbindings.host` → withdrawn; its concerns moved into binding-invoker and the document-store-backed context loop.
...
- **binding-invoker** is the runtime workhorse that invokes an operation's binding. When a binding needs something the caller has not supplied (credentials, a session, configuration), the invoker raises a `CONTEXT_REQUIRED` challenge reporting its target; the runtime resolves it, persists durable results in a store (a **document-store**), and retries. Authentication lives entirely in this loop, never in the OBI document. The store is generic; the context meaning lives in this contract.
