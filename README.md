...
An interface's identity is the URL it is retrieved from (spec §9). The canonical location for each is a stable openbindings.com path:

    https://openbindings.com/interfaces/<name>/<version>.json

for example `https://openbindings.com/interfaces/kv-store/0.1.json`. Point tooling there to compare or generate against a contract (`ob compat <url> <candidate>`, `fetchInterface(url)`). The directory layout in this repository mirrors those paths.
