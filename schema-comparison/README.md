...
5. **`allOf` flattening.** All branches merge into a single schema (rules below), then the merged result is normalized. `oneOf`/`anyOf` *inside* an `allOf` branch fails closed.
